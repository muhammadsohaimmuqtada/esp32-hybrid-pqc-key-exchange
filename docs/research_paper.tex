...
\begin{enumerate}
    ...
    \end{aligned}
    \end{equation}
    where the HKDF salt is set to the ASCII string\\
...
We evaluated the performance of the hybrid protocol against standalone classical and PQC modes over 100 consecutive end-to-end iterations on our physical testbed, with the high-level protocol mode comparisons detailed in Table \ref{tab:protocol_comparison}. A full statistical confidence breakdown across the 100 iterations is provided in Table \ref{tab:statistical_confidence}, while the specific hardware and software configuration used for all benchmarks is documented in Table \ref{tab:hw_config}. Measurements for latency and CPU cycles were captured using native hardware timers (\texttt{esp\_timer\_get\_time()}).
...
Table \ref{tab:mbedtls} reports the raw instrumented output of this standalone benchmark, captured via native hardware timers over the ESP32's serial interface during a live handshake execution. These figures corroborate the mbedTLS baseline row already summarized in Table \ref{tab:comparison}.
...
The complete source code of the firmware, benchmark scripts, and the Python server implementation, along with the captured PCAP files and raw telemetry logs, are openly available in the repository at \url{https://github.com/muhammadsohaimmuqtada/esp32-hybrid-pqc-key-exchange}.
