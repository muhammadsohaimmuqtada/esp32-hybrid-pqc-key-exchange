...
The case for running two KEMs simultaneously rather than picking one is theoretically grounded: Bindel et al.~\cite{b8} formally proved that a combiner KEM is secure as long as at least one constituent KEM is secure. Standardized guidance in NIST SP~800-227~\cite{b9}, hybrid terminology RFC~9794~\cite{b31}, HPKE RFC~9180~\cite{b32}, and ongoing IETF drafts~\cite{b16,b17} all recommend or frame hybrid designs for exactly this reason during the transition period. Our implementation directly applies this combiner pattern.
...
What is left: there is no empirical TVLA or DPA characterization of this implementation. The code uses constant-time primitives, but side-channel leakage is not yet measured~\cite{b18,b40}. That is the main direction we would pursue next.


\section*{Data and Code Availability}
The complete source code of the firmware, benchmark scripts, ProVerif models, Known Answer Tests, and the Python server implementation, along with raw serial logs and telemetry datasets, are openly available on GitHub at \url{https://github.com/muhammadsohaimmuqtada/esp32-hybrid-pqc-key-exchange} on branch \texttt{main}, commit \texttt{a39a0d8} and successors. Packet traces in \texttt{data/sanitized\_pcaps/} are synthetic Scapy illustrations of message structure.
