...
\end{equation}
The payload consists of a 1-byte mode identifier (Hybrid = 2), a 32-byte X25519 public key, an 800-byte ML-KEM-512 public key, and a 32-byte HMAC signature, totalling 865 bytes.
...
\end{equation}
The payload consists of a 16-byte Session ID, a 32-byte X25519 public key, a 768-byte ML-KEM-512 ciphertext, and a 32-byte HMAC signature, totalling 848 bytes.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.65\columnwidth]{architecture.png}}
\caption{System Architecture Diagram}
\label{fig1}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.65\columnwidth]{sequence.png}}
...
\hline
\hline
X25519-only & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
ML-KEM-only & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
Hybrid X25519 + ML-KEM-512 & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
TLS 1.3 baseline & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
\end{tabular}%
}
...
\hline
\hline
Handshake latency & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
SRAM usage & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
CPU cycles & [TBD] & [TBD] & [TBD] & [TBD] & [TBD] \\
\hline
...
\hline
\hline
ESP32 model & [Exact model number] \\
\hline
CPU clock & [240 MHz or actual configured value] \\
\hline
ESP-IDF version & [Add exact version] \\
\hline
mbedTLS version & [Add exact version] \\
\hline
ML-KEM library & [Add library/source and version] \\
\hline
Compiler flags & [Add optimization flags such as -O2/-Os] \\
\hline
Wi-Fi RSSI & [Add measured signal strength during tests] \\
\hline
Server hardware & [Add CPU/RAM/OS details] \\
\hline
Distance/router & [Add test distance and router model] \\
\hline
\end{tabular}%
}
\label{tab:hw_config}
...
To establish a baseline, we executed a standard mbedTLS 1.3 handshake on the same ESP32 hardware over the network. The TLS 1.3 baseline was evaluated using mbedTLS with a 2048-bit RSA self-signed certificate, running at 240 MHz CPU frequency on Core 1 with identical task priority as the hybrid protocol, connecting to a live TLS 1.3 endpoint (\texttt{example.com:443}) to obtain a genuine, network-executed handshake. We note that comparing our local hybrid protocol to a public Internet endpoint inherently inflates the network RTT of the baseline; however, the critical metric of comparison in this evaluation is the \textbf{cryptographic processing overhead} and \textbf{raw CPU cycle count} on the ESP32, rather than the absolute network transit time. This baseline consumed $776.40$ ms (approximately 1.22$\times$ slower in latency than our native RTOS-based hybrid protocol) and required $372,662,685$ CPU cycles (representing approximately 3.60$\times$ more processor overhead).
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.65\columnwidth]{power.png}}
